...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ans = [p for p in paragraphs if select(p)]
    if k < len(ans):
        return ans[k]
    return ""
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    if typed_len == source_len == 0:
        return 100.0
    if typed_len == 0:
        return 0.0
    correct = 0
    for i in range(min(typed_len, source_len)):
        if typed_words[i] == source_words[i]:
            correct += 1
    return correct / typed_len * 100.0
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        # BEGIN PROBLEM EC
        "*** YOUR CODE HERE ***"
        if ((typed, source) not in cache) or (cache[(typed, source)] <= limit):
...
def furry_fixes(typed, source, limit):
    ...
    def inner(t, s, cnt):
        ...
        if t[0] == s[0]:
            return inner(t[1:], s[1:], cnt)
        return inner(t[1:], s[1:], cnt + 1)
...
def minimum_mewtations(typed, source, limit):
    ...
    index = find_index(typed, source)
    if index:
        return minimum_mewtations(typed[index:], source[index:], limit)
    else:
...
def report_progress(typed, source, user_id, upload):
    ...
    "*** YOUR CODE HERE ***"
    index = 0
    while index < len(typed) and typed[index] == source[index]:
        index += 1
    report = {"id": user_id, "progress": index / len(source)}
    upload(report)
...
def time_per_word(words, timestamps_per_player):
    ...
    return words, [
        [
            timestamps_per_player[i][j] - timestamps_per_player[i][j - 1]
...
def fastest_words(match_object):
    ...
    def get_fastest(index):
        fastest = 0
        for i in player_indices:
            if times[i][index] < times[fastest][index]:
                fastest = i
        return fastest

    fast = [get_fastest(i) for i in word_indices]
    return [[words[j] for j in word_indices if fast[j] == i] for i in player_indices]
    # END PROBLEM 11
