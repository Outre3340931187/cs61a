...
class Player:
    ...
    def speech(self, other):
        ...
        lose = other.popularity // 10
        self.votes += gain
        self.popularity += gain
        other.popularity -= lose
...
def level_mutation_link(t, funcs):
    ...
    >>> t2    # Level 0: 1+1=2; Level 1: 2*5=10, 3*5=15; Level 2 (leaf): 4**2=16
    ...
    if not t.branches and remaining is not Link.empty:
...
def crispr_gene_insertion(lnk_of_genes, insert):
    ...
    def solve(link, times):
        temp = link
        while temp is not Link.empty:
            if temp.first == starter:
